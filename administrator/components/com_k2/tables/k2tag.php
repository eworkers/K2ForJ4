...
class TableK2Tag extends K2Table
{
    ...
    function check()
    {
        ...
        $this->name = StringHelper::str_ireplace('-', '', $this->name);
        $this->name = StringHelper::str_ireplace('.', '', $this->name);
        ...
        if ($params->get('k2TagNorm')) {
            ...
            //$this->name = StringHelper::str_ireplace($searches, $replacements, $this->name); // This causes character stripping in J!1.5!!
            $this->name = str_ireplace($searches, $replacements, $this->name);

            // Switch case
            if ($params->get('k2TagNormCase') == 'upper') {
                $this->name = StringHelper::strtoupper($this->name);
            } else {
                $this->name = StringHelper::strtolower($this->name);

                // Special case for Greek letter s final
                $this->name = StringHelper::str_ireplace('σ ', 'ς ', $this->name);
                if (StringHelper::substr($this->name, -1) == 'σ') {
                    $this->name = StringHelper::substr($this->name, 0, -1);
                    $this->name .= 'ς';
                }
            }
        }
        ...
        $this->name = StringHelper::trim($this->name);
        if ($this->name == '') {
            $this->setError(Text::_('K2_TAG_CANNOT_BE_EMPTY'));
            return false;
        }

        if (strlen(utf8_decode($this->name)) < 2) {
            $this->setError(Text::_('K2_TAG_CANNOT_BE_A_SINGLE_CHARACTER'));
            return false;
        }
        ...
        if (!$this->id) {
            ...
            if ($this->_db->loadResult()) {
                $this->setError(Text::_('K2_THIS_TAG_EXISTS_ALREADY'));
                return false;
            }
        }
