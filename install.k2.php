...
if (version_compare(JVERSION, '1.6.0', '<')) {
    ...
    if (is_a($modules, 'SimpleXMLElement') && count($modules->children())) {
    ...
    if (Folder::exists(JPATH_ADMINISTRATOR.'/components/com_joomfish/contentelements')) {
        $elements = $this->manifest->getElementByPath('joomfish');
        if (is_a($elements, 'SimpleXMLElement') && count($elements->children())) {
